...
if($get_item!=false){
	...
	$div_content.='<div class="large-4 columns"><a href="#" class="tiny button items " item_id="'.$get_item[$item_index]["id"].'" item_details="'.$get_item[$item_index]["name"].'/'.$get_item[$item_index]["rate"].'/'.$get_item[$item_index]["tax"].'">'.$get_item[$item_index]["name"].'  Rs .'.$get_item[$item_index]["rate"].'</a></div>';
	$item_index++;
	 } 
$div_content.='</div></div>';
print $div_content;
